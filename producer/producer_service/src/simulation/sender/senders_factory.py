...
class SenderFactory:
    ...
    def create_app_log_sender(server_id:str, serializer_factory: ISerializerFactory, kafka_producer: Producer) -> Sender:
        return Sender(
            ...
            server_id=server_id,
            topic="servers.logs.application",
        )
    
    def create_access_log_sender(server_id: str, serializer_factory: ISerializerFactory, kafka_producer: Producer) -> Sender:
        return Sender(
            msg_factory=AccessLogRandomGeneration(),
            k_producer=kafka_producer,
            serializer=serializer_factory.create_access_log_serializer(),
            server_id=server_id,
            topic="servers.logs.access",
        )
    
    def create_metrics_sender(server_id: str, serializer_factory: ISerializerFactory, kafka_producer: Producer) -> Sender:
        return Sender(
            msg_factory=ServerMetricsRandomGeneration(),
            k_producer=kafka_producer,
            serializer=serializer_factory.create_metrics_serializer(),
            server_id=server_id,
            topic="servers.metrics",
        )